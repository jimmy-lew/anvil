...
function discord_tag(input: string) {
...
export async function find_member(guild: Guild, input: string) {
  const discord_id = is_discord_id(input)
  if (discord_id)
    return (await guild.members.fetch(discord_id))
...
export async function find_role(guild: Guild, input: string) {
  const discord_id = is_discord_id(input)
  if (discord_id)
    return (await guild.roles.fetch(discord_id))
